...
#### read in data without woody species ####
...
all.data = imputed.NW %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
annual.data = imputed.NW.annual %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
perennial.data = imputed.NW.perennial %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
grass = imputed.NW.grass %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
forb = imputed.NW.forb %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
grass.perennial = imputed.NW.perennial.grass %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
forb.annual = imputed.NW.annual.forb %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()
forb.perennial = imputed.NW.perennial.forb %>%
  dplyr::select(cover.change,leafN.final:mean.MAP) %>%
  drop_na()

#### determining best parameter combination to generate 1000 trees for non-woody data ####
# bag fraction of 0.50 and 0.75, step.size of 25 and 50, tc = 10, 
...
#### determining best tree complexity to use for non-woody data ####
...
ggPerformance(all.data.map.dsi)
# 1250 trees Per.Expl = 6.27%

#saveRDS(all.data.map.dsi, file = "./Formatted.Data/Revisions/all.species.model.NW.rds")

ggInfluence(all.data.map.dsi)
# leafN, MAP, height, are significant
...
gbm.interactions(all.data.map.dsi)$interactions
ggInteract_list(all.data.map.dsi, index = T)
# 2 height x 1 leafN 3.99
# 10 DSI x 1 leafN 2.75
# 11 MAP x 1 leafN 2.65
# 3 rootN x 2 height 1.20
# 11 MAP x 4 SLA 0.36
# 9 RMF x 1 leafN 0.35


# Randomization of the response to test significance of the three strongest interactions. Note, the parameters need to be the same as your BRT model
Interact_boot_dsi<-ggInteract_boot(c('height.final','leafN.final'),c('mean.DSI','leafN.final'),
                                   c('mean.MAP','leafN.final'),c('rootN.final','height.final'),
                                   c('mean.MAP','SLA.final'),c('RMF.final','leafN.final'),
                                   nboots = 500, data=all.data, predictors =  c(2:12), 
                                   response="cover.change",
                                   family = "gaussian", tc = 4, lr = 0.0005, bf= 0.50, global.env=F)

# Significance histogram p-value< 0.05
ggInteract_boot_hist(data = Interact_boot_dsi, column = 2,obs = 3.99) # significant
# 2 height x 1 leafN 3.99
ggInteract_boot_hist(data = Interact_boot_dsi, column = 3,obs = 2.75) # significant
# 10 DSI x 1 leafN 2.75
ggInteract_boot_hist(data = Interact_boot_dsi, column = 4,obs = 2.65) # significant
# 11 MAP x 1 leafN 2.65
ggInteract_boot_hist(data = Interact_boot_dsi, column = 5,obs = 1.20) 
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 0.36)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 0.25)
...
#### annual data non-woody ####
annual.no.site.map.dsi = gbm.step(data=annual.data, gbm.x = c(2:12), gbm.y=1,
                                  family = "gaussian", tree.complexity = 3, learning.rate = 0.0001,
                                  bag.fraction = 0.50, n.trees = 50, verbose = TRUE, step.size = 50)

ggPerformance(annual.no.site.map.dsi)
# 1350 trees Per.Expl = 2.57%

#saveRDS(annual.no.site.map.dsi, file = "./Formatted.Data/Revisions/annual.species.model.NW.rds")
...
gbm.interactions(annual.no.site.map.dsi)$interactions
ggInteract_list(annual.no.site.map.dsi, index = T)
# 10 DSI x 1 leafN 0.30
# 11 MAP x 1 leafN 0.19
# 11 MAP x 2 height 0.11
# 11 MAP x 66RTD 0.05
# 11 MAP x 6 rootDiam 0.05
# 11 MAP x 4 SLA 0.02
...
Interact_boot_dsi<-ggInteract_boot(c('mean.DSI','leafN.final'),c('mean.MAP','leafN.final'),
                                   c('mean.MAP','height.final'),c('mean.MAP','RTD.final'),
                                   c('mean.MAP','rootDiam.final'),c('mean.MAP','SLA.final'),
                                   nboots = 500, data=annual.data, predictors =  c(2:12), 
                                   response="cover.change",
                                   family = "gaussian", tc = 3, lr = 0.0001, bf= 0.50, global.env=F)

# Significance histogram p-value< 0.05
ggInteract_boot_hist(data = Interact_boot_dsi, column = 2,obs = 0.30)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 3,obs = 0.19)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 4,obs = 0.11) 
ggInteract_boot_hist(data = Interact_boot_dsi, column = 5,obs = 0.05) 
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 0.05)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 0.02)
...
ggPerformance(perennial.data.map.dsi)
# 2050 trees Per.Expl = 10.14%

#saveRDS(perennial.data.map.dsi, file = "./Formatted.Data/Revisions/perennial.species.model.NW.rds")
...
gbm.interactions(perennial.data.map.dsi)$rank.list
ggInteract_list(perennial.data.map.dsi)
# 2 height x 1 leafN 36.69
# 10 DSI x 1 leafN 6.93
# 11 MAP x 1 leafN 1.38
# 8 RTD x 7 SRL 1.23
# 10 DSI x 2 height 0.87
# 5 depth x 1 leafN 0.68

# Randomization of the response to test significance of the three strongest interactions. Note, the parameters need to be the same as your BRT model

Interact_boot_perennial_dsi<-ggInteract_boot(c('height.final','leafN.final'),c('mean.DSI','leafN.final'),
                                             c('mean.MAP','leafN.final'),c('RTD.final','SRL.final'),
                                             c('mean.DSI','height.final'),c('root.depth.final','leafN.final'),
                                             nboots = 500, data=perennial.data, predictors =  c(2:12), 
                                             response="cover.change",
                                             family = "gaussian", tc = 3, lr = 0.0005, bf= 0.75, global.env=F)

# Significance histogram p-value<0.05)
ggInteract_boot_hist(data = Interact_boot_perennial_dsi, column = 2,obs = 36.69) # significant
# 2 height x 1 leafN 36.69
ggInteract_boot_hist(data = Interact_boot_perennial_dsi, column = 3,obs = 6.93) # significant
# 10 DSI x 1 leafN 6.93
ggInteract_boot_hist(data = Interact_boot_perennial_dsi, column = 4,obs = 1.38) # non-significant
ggInteract_boot_hist(data = Interact_boot_perennial_dsi, column = 5,obs = 1.23) # non-significant
ggInteract_boot_hist(data = Interact_boot_perennial_dsi, column = 6,obs = 0.87) # non-significant
ggInteract_boot_hist(data = Interact_boot_perennial_dsi, column = 7,obs = 0.68) # non-significant
...
ggPerformance(forb.map.dsi)
# 1150 trees Per.Expl = 15.29%

#saveRDS(forb.map.dsi, file = "./Formatted.Data/Revisions/forb.species.model.NW.rds")

ggInfluence(forb.map.dsi)
# leafN, RTD, DSI, height
...
gbm.interactions(forb.map.dsi)$interactions
ggInteract_list(forb.map.dsi, index = T)
# 8 RTD x 1 leafN 31.63
# 10 DSI x 1 leafN 6.10
# 8 RTD x 7 SRL 5.79
# 11 MAP x 1 leafN 4.80
# 2 height x 1 leafN 2.84
# 6 rootDiam x 1 leafN 2.42


# Randomization of the response to test significance of the three strongest interactions. Note, the parameters need to be the same as your BRT model
Interact_boot_dsi<-ggInteract_boot(c('RTD.final','leafN.final'),c('mean.DSI','leafN.final'),
                                   c('RTD.final','SRL.final'),c('mean.MAP','leafN.final'),
                                   c('height.final','leafN.final'),c('rootDiam.final','leafN.final'),
                                   nboots = 500, data=forb, predictors =  c(2:12), 
                                   response="cover.change",
                                   family = "gaussian", tc = 8, lr = 0.0005, bf= 0.75, global.env=F)

# Significance histogram p-value< 0.05
ggInteract_boot_hist(data = Interact_boot_dsi, column = 2,obs = 31.63) # significant
# 8 RTD x 1 leafN 31.63
ggInteract_boot_hist(data = Interact_boot_dsi, column = 3,obs = 6.10)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 4,obs = 5.79) 
ggInteract_boot_hist(data = Interact_boot_dsi, column = 5,obs = 4.80) 
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 2.84)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 2.42)


#### Grass Perennials non-woody ####
grass.perennial.map.dsi  = gbm.step(data=grass.perennial, gbm.x = c(2:12), gbm.y=1,
                                   family = "gaussian", tree.complexity = 2, learning.rate = 0.0001,
                                   bag.fraction = 0.5, n.trees = 50, verbose = TRUE, step.size = 50)

ggPerformance(grass.perennial.map.dsi)
# 1800 trees Per.Expl = 2.74%

#saveRDS(grass.perennial.map.dsi, file = "./Formatted.Data/Revisions/grass.perennial.model.NW.rds")

ggInfluence(grass.perennial.map.dsi)
# leafN, SRL, rootN, RMF, height, MAP
...
gbm.interactions(grass.perennial.map.dsi)$interactions
ggInteract_list(grass.perennial.map.dsi, index = T)
# 10 DSI x 2 height 0.04
# 11 MAP x 9 RMF 0.03
# 11 MAP x 7 SRL 0.02
# 10 DSI x 1 leafN 0.02
# 10 DSI x 9 RMF 0.01
# 9 RMF x 6 rootDiam 0.01



# Randomization of the response to test significance of the three strongest interactions. Note, the parameters need to be the same as your BRT model
Interact_boot_dsi<-ggInteract_boot(c('mean.DSI','height.final'),c('mean.MAP','RMF.final'),
                                   c('mean.MAP','SRL.final '),c('mean.DSI','leafN.final'),
                                   c('mean.DSI','RMF.final'),c('RMF.final','rootDiam.final '),
                                   nboots = 500, data=grass.perennial, predictors =  c(2:12), 
                                   response="cover.change",
                                   family = "gaussian", tc = 2, lr = 0.0001, bf= 0.50, global.env=F)

# Significance histogram p-value< 0.05
ggInteract_boot_hist(data = Interact_boot_dsi, column = 2,obs = 0.04)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 3,obs = 0.03)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 4,obs = 0.02)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 5,obs = 0.02)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 6,obs = 0.01)
ggInteract_boot_hist(data = Interact_boot_dsi, column = 7,obs = 0.01)


#### Forb Perennial non-woody ####
forb.perennial.map.dsi = gbm.step(data=forb.perennial, gbm.x = c(2:12), gbm.y=1,
                                  family = "gaussian", tree.complexity = 3, learning.rate = 0.0005,
                                  bag.fraction = 0.5, n.trees = 50, verbose = TRUE, step.size = 50)

ggPerformance(forb.perennial.map.dsi)
# 1000 trees Per.Expl = 7.63%

#saveRDS(forb.perennial.map.dsi, file = "./Formatted.Data/Revisions/forb.perennial.model.NW.rds")

ggInfluence(forb.perennial.map.dsi)
# height, RTD, leafN, DSI
...
gbm.interactions(forb.perennial.map.dsi)$interactions
ggInteract_list(forb.perennial.map.dsi, index = T)
# 2 height x 1 leafN 0.11
# 10 DSI x 1 leafN 0.10
# 10 DSI x 8 RTD 0.07
# 10 DSI x 5 root.depth 0.07
# 7 SRL x 4 SLA 0.06
# 9 RMF x 4 SLA 0.05
...
# Randomization of the response to test significance of the three strongest interactions. Note, the parameters need to be the same as your BRT model
Interact_boot_dsi<-ggInteract_boot(c('height.final','leafN.final'),c('mean.DSI','leafN.final'),
                                   c('mean.DSI','RTD.final'),c('mean.DSI','root.depth.final'),
                                   c('SRL.final','SLA.final'),c('RMF.final','SLA.final'),
                                   nboots = 500, data=forb.perennial, predictors =  c(2:12), 
                                   response="cover.change",
